...
ChatGPT 解锁判定以“地区是否受限”和“HTTPS 链路是否完成”为核心，不把命令行
探测遇到的反自动化响应当成解锁失败：

- `api.openai.com` 返回 401，表示服务可达但请求没有登录凭据，属于正常可用；
- `chatgpt.com` 或认证入口返回 403，且响应中没有地区限制标记，视为反自动化保护；
- 静态资源域名根路径返回 404，表示主机可达但该根路径没有资源；
- 响应正文明确包含地区不支持标记时，仍判定为 `region_limited`；
- 超时、DNS/TCP/TLS 失败或 HTTP 5xx 才会降低链路可用判定。

因此公开页面使用“地区可用”而不是“完整可用”：它确认出口地区和网络链路可以访问
ChatGPT，但不模拟真实浏览器登录，也不判断具体账号状态。

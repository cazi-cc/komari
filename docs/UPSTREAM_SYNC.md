...
- ChatGPT 解锁任务仍只允许内置服务清单，默认 60 秒轻量检测和 15 分钟完整校验；
- 解锁线路公开快照仍不包含域名、解析地址、DNS、固定入口或地址指纹，访客查询不会触发 Agent 探测；
- 解锁快照仍按 1/5/15 分钟分级刷新且不读取私有压缩诊断载荷，Telegram 只在连续两轮异常或恢复后通知；
